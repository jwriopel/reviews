...
def hunks(u_diff):
    """
    Parse the hunks out of the unified diff.

    :param u_diff: File-like object containing the unified diff.
    :return: List containig pairs of tuples with hunk information.
    """
    d_hunks = []

    clean_lines = (l.strip() for l in u_diff.readlines())
    hunk_lines = (l for l in clean_lines if l.startswith("@@"))

    for line in hunk_lines:
        h_res = HUNK_RE.search(line)
        if h_res:
            d_hunks.append(h_res.groups())
